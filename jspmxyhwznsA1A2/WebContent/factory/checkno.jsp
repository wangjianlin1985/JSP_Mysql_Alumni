...
<%
	...
	if (valuecopy.length() > 1)
		valuecopy = valuecopy.substring(0, valuecopy.length() - 1);
	value = valuecopy;
	...
	if (checktype.equals("iinsert")) {
		...
		List list = new CommDAO().select("select * from fangyd where fno='" + fno + "' and stime<='" + value
				+ "' and etime>='" + value + "'");
		if (list.size() > 0) {
			out.print("Y");
		} else {
	...
	if (checktype.equals("update")) {
		...
		value = Info.getGBKStr(value);
		System.out.println(value);
		List list = new CommDAO()
				.select("select * from " + table + " where " + col + "='" + value + "' and id!=" + id);
